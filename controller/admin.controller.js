...
module.exports.getAllUsers = catchAsyncError(async (req, res) => {
  ...
  const totalUsers = countResults.rows[0].count;
  const usersPerPage = 10;

  const offset = (page - 1) * usersPerPage;
  const usersResult = await database.query(
    `select * from users where role=$1 order by created_at DESC limit $2 offset $3`,
    ["user", usersPerPage, offset],
  );
...
//delete user
module.exports.deleteUser = catchAsyncError(async (req, res) => {
  const { id } = req.params;
  const deleteResult = await database.query(`delete from users where id=$1`, [
    id,
  ]);
...
//dashboard
module.exports.dashboardStats = catchAsyncError(async (req, res, next) => {
